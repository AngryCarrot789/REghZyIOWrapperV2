...
namespace REghZyIOWrapperV2.SocketTest.Server {
    class Program {
        public static void Main(string[] args) {
            ...
            Console.WriteLine("Getting localhost...");
            Socket socket = SocketConnector.CreateServerSocket(new IPEndPoint(IPAddress.Loopback, 1440), AddressFamily.InterNetwork);
            socket.Listen(10);
            Console.WriteLine($"localhost = {IPAddress.Loopback.ToString()}");

            Console.WriteLine("Creating server system on localhost@1440");
            ThreadPacketSystem system = SocketConnector.AcceptClientConnection(socket);
